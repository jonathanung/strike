...
| `internal/permission` | Ordered allow/ask/deny rulesets, last-match-wins; ask service; **Explain** (matched rule + layer); scoped grants with TTL; shipped presets (`read-only`/`dev`/`yolo-with-sandbox`); `permission.decided` audit events; **tool-chain correlator** (content-free multi-step ask/deny within a turn, #891); `CompileSandbox` maps write/edit denials + webfetch/websearch/mcp network posture into `sandbox.Policy` | `protocol`, `tool` (for `AskRequest`), `sandbox`, stdlib |
