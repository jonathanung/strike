...
| `internal/secret` | Shared credential detection/redaction + secret-ref env indirection (`Redact`, `RedactEvent`, `ScrubToolOutput`, `ParseRef`/`Resolve`/`MergeEnv`). Used by session persist, engine tool scrub, TUI `/export`, MCP errors; #790 timeline export should import this package | stdlib + `protocol` (event redaction only) |
...
- `internal/tui/...`: no `internal/*` imports except `protocol`, `host`,
  `secret`, `tui/...`. (`pkg/protocol` is also allowed — it is not under
  `internal/`.) `secret` is a pure redaction helper (no auth/config I/O).
