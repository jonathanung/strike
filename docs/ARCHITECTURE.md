...
| `internal/integrate/mcp` | MCP client (stdio + streamable HTTP) + session manager; admission before tool bind; bridges tools onto `tool.Registry` as `mcp_<server>_<tool>`; retry/disable/quarantine; tools-only stdio **server** (`Server`) for `strike mcp-serve` | `tool`, `admission`, `secret`, stdlib, net/http |
