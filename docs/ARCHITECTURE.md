...
│
├── tees every Event through internal/persist/session before it reaches the TUI:
...
| `internal/trust/security` | Shared `Finding` + `Severity` for trust-boundary scanners (admission #889, content guards #890) | stdlib only |
| `internal/trust/admission` | Register/load-time admission policy (permissive/default/strict severity→allow\|warn\|block\|quarantine), MCP/skill/plugin scanners, home-anchored allow-paths | `security`, `pkg/redact`, stdlib |
...
| `internal/persist/ledger` | Shared decision/assumption/constraint ledger (append/invalidate/supersede/revalidate; optional assumption evidence pins; history preserved); JSON under `~/.strike/ledger/`. Active slice auto-loads into system prompt; `ledger.updated` on mutations. Prefer over burying critical assumptions only in chat | stdlib |
