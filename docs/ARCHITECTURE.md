...
| `internal/sandbox` | OS-primitive process sandbox: `Wrap(argv, Policy)` via Linux `bwrap` / macOS `sandbox-exec`; Policy carries mode, write denials, `NoNetwork` (host net on by default), and optional `NetworkAllow` host/CIDR list (webfetch/websearch; shared shape for future container net); `Explain`/`ProfileText` for `/sandbox explain`; graceful degrade + startup warning when unavailable | stdlib only |
| `internal/container` | **E12 container engine (#547 / #582):** CLI shell-out to `docker`/`podman` (no Moby SDK). Injectable `ExecFunc`; `Runtime` (`Available`/`Pull`/`Create`/`Start`/`Stop`/`Remove`/`InspectID`/`Copy*`/`Exec`); deterministic `strike-<repo>-<sha256[:16]>` names + `com.strike.*` labels. Higher-level Manager/build/launch/attach land in E12.1+. `internal/tui` must not import this package | stdlib only (`os/exec`) |
