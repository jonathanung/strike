...
| `internal/engine` | Headless agent runtime: turn loop, tool dispatch, permission/question integration, deferred agent switch; implicit session-scoped agent **team** (lead + children roster in `team.go`) | `protocol`, `provider`, `tool`, `permission`, `question`, `memory`, `config` |
