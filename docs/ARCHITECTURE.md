...
| `internal/permission` | Ordered allow/ask/deny rulesets, last-match-wins; ask service; **Explain** (matched rule + layer); scoped grants with TTL; shipped presets (`read-only`/`dev`/`yolo-with-sandbox`); `permission.decided` audit events; `CompileSandbox` maps write/edit denials + webfetch/websearch/mcp network posture into `sandbox.Policy` | `protocol`, `tool` (for `AskRequest`), `sandbox`, stdlib |
...
| `internal/plugin` | Versioned plugin bundle discovery + manifest validation + path confinement + enablement; passive contribution file refs (agents/skills/workflows/themes/providers). No executable activation | `version`, stdlib |
