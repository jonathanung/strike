...
| 5 | TB eval uses `--dangerously-skip-permissions` | HARNESS-TOOL | WithTBEvalExecDefaults; network.allow still written | — | **22/25** ($20) 0 errors | — | **retain (requested)** | DSP + isolation: host net allowed. **gcode-to-text resolved**. Lost dna-assembly/insert + raman vs i3 24/25 (variance). SWE stays `--auto`. |
