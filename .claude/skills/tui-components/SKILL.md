...
- **Edit `internal/tui/app/_src/<group>/` only** for package `tui`. Flattened
  `internal/tui/app/*.go` are gitignored and regenerated by
  `go generate ./internal/tui/app` / make / CI — edits there are silently lost.
...
- Components are pure renderers: exported rendering functions take a
  ...
  in `internal/tui/app` model files.
