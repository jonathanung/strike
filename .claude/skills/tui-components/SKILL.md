...
- **Edit `internal/tui/_src/<group>/` only** for package `tui`. Flattened
  `internal/tui/*.go` are gitignored and regenerated by
  `go generate ./internal/tui` / make / CI — edits there are silently lost.
...
- `internal/tui/...` may import only `internal/protocol`, `internal/host`,
  ...
  enforces this. Charm modules: v1 `github.com/charmbracelet/…` or v2
