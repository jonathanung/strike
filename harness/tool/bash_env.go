...
import (
	...
	"github.com/jonathanung/strike-cli/harness/secretref"
)
...
func bashEnv(tc *Context) ([]string, error) {
	...
	refs := make(map[string]secretref.Ref, len(tc.BashSecrets))
	...
	merged, err := secretref.MergeEnv(base, refs)
	if err != nil {
		return nil, err
	}
