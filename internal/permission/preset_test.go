...
func TestPresetsShipped(t *testing.T) {
	...
	if Evaluate("status", "*", Defaults(), ro.Rules) != Allow {
		t.Fatal("read-only should allow status")
	}
	if Evaluate("verify", "B", Defaults(), ro.Rules) != Deny {
		t.Fatal("read-only should deny verify")
	}
