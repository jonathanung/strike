...
func TestExternalRejectsMalformedJSON(t *testing.T) {
	tests := []struct {
		...
		{"missing", "exited without turn.complete"},
	...
	for _, tt := range tests {
		t.Run(tt.mode, func(t *testing.T) {
			...
			_, err := h.Run(context.Background(), harness.Request{TurnID: "turn-1"})
...
func TestExternalCompleteDoesNotWaitForLiveHarness(t *testing.T) {
	...
	started := time.Now()
	result, err := h.Run(context.Background(), harness.Request{TurnID: "turn-1"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if elapsed := time.Since(started); elapsed > 2*time.Second {
		t.Fatalf("Run took %v after turn.complete", elapsed)
	}
}
...
func TestExternalCancellationStopsLiveHarness(t *testing.T) {
	...
	defer cancel()
	started := time.Now()
	_, err := h.Run(ctx, harness.Request{TurnID: "turn-1"})
...
func TestHarnessHelperProcess(t *testing.T) {
	...
	if mode == "version" {
		fmt.Println(`{"version":2,"type":"turn.complete","turnId":"turn-1"}`)
		return
	}
	if mode == "unknown" {
		fmt.Println(`{"version":1,"type":"future.message","turnId":"turn-1"}`)
		return
	}
	if mode == "missing" {
		os.Exit(0)
	...
	if mode == "duplicate" {
		fmt.Println(`{"version":1,"type":"provider.call","turnId":"turn-1","callId":"same","request":{"model":"candidate","messages":[]}}`)
		fmt.Println(`{"version":1,"type":"tool.execute","turnId":"turn-1","toolCallId":"same","name":"x","arguments":{}}`)
		time.Sleep(time.Hour)
	}
	if mode == "stays-alive" {
		fmt.Println(`{"version":1,"type":"turn.complete","turnId":"turn-1","text":"final"}`)
		time.Sleep(time.Hour)
	}
	if mode == "blocks" {
		time.Sleep(time.Hour)
	...
	if mode == "tool" {
		fmt.Println(`{"version":1,"type":"tool.execute","turnId":"turn-1","toolCallId":"tool-7","name":"allowed","arguments":{"value":1}}`)
		for scanner.Scan() {
			var m struct {
			...
			if json.Unmarshal(scanner.Bytes(), &m) == nil && m.Type == "tool.result" {
				fmt.Printf(`{"version":1,"type":"turn.complete","turnId":"turn-1","text":%q}`+"\n", m.Output)
				return
			}
		}
		return
	}
	fmt.Println(`{"version":1,"type":"progress.emit","turnId":"turn-1","payload":{"message":"working"}}`)
	for _, id := range []string{"a", "b"} {
		fmt.Printf(`{"version":1,"type":"provider.call","turnId":"turn-1","callId":%q,"request":{"model":"candidate","messages":[]}}`+"\n", id)
	}
	completions := 0
	for scanner.Scan() {
		var m struct {
		...
		if json.Unmarshal(scanner.Bytes(), &m) == nil && m.Type == "provider.event" && (m.Kind == "completion" || m.Kind == "error") {
			completions++
			if completions == 2 {
				fmt.Println(`{"version":1,"type":"turn.complete","turnId":"turn-1","text":"final","stopReason":"end_turn"}`)
				return
			}
		}
	}
