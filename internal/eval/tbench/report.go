...
func LoadReport(path string) (Report, error) {
	...
	if err := json.Unmarshal(data, &rep); err != nil {
		return Report{}, fmt.Errorf("tbench: report %s: %w", path, err)
	}
	return rep, nil
}
...
func FormatReport(rep Report) string {
	...
	fmt.Fprintf(&buf, "Terminal-Bench subset report (#562) schema=%s run=%s\n",
		rep.SchemaVersion, rep.RunID)
	fmt.Fprintf(&buf, "provider=%s model=%s grader=%s pin=%s generated=%s\n",
		rep.Provider, rep.Model, rep.Grader, rep.DatasetPin, rep.GeneratedAt.Format(time.RFC3339))
	...
	fmt.Fprintln(w, "status\tinstance\twall_ms\ttok_in\ttok_out\tcost\treward\tdetail")
	for _, r := range rep.Results {
		detail := r.Error
		if detail == "" {
		...
		fmt.Fprintf(w, "%s\t%s\t%d\t%d\t%d\t%.4f\t%.2f\t%s\n",
			r.Status, r.InstanceID, r.WallClockMs, r.TokensIn, r.TokensOut, r.CostUSD, r.Reward, detail)
	}
	_ = w.Flush()
	return buf.String()
}
