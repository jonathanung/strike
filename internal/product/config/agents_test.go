...
func TestLoadAgentsAndSkills(t *testing.T) {
	...
	for _, want := range []string{"build", "plan", "explore", "general", "commit", "reviewer", "tester", "debugger", "validator", "orchestrator", "pr-babysitter"} {
...
func TestLoadSkillsWithErrorRejectsUnsafeAndReservedNamesClearly(t *testing.T) {
	tests := []struct {
		...
		{name: "reserved ftue", fileName: "ftue.md", wantDetail: `skill name "ftue" is reserved`},
...
func TestParseAgentCapabilities(t *testing.T) {
	got := parseAgentCapabilities(map[string]string{
		"capabilities": "Explore, search",
		"specialty":    "scout",
	})
	want := []string{"explore", "search", "scout"}
	if len(got) != len(want) {
	...
	if parseAgentCapabilities(nil) != nil {
		t.Fatal("nil meta")
	}
}
