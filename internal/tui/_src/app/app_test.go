...
func TestPaletteHelpInvocationOpensHelpModal(t *testing.T) {
	...
	for _, want := range []string{"/session", "/rename", "/export", "/timeline", "/copy", "/theme", "/memory", "/queue", "/issues", "/compact", "/fast", "/think", "/layout", "/md-read", "/keys", "/legend", "/settings", "/exit", "/quit", "/palette", "/interrupt", "/agent-next", "/focus-left"} {
...
func TestRootSwitcherOpensModal(t *testing.T) {
	...
	if !ok {
		t.Fatalf("modal = %T, want *rootSwitcherModal", m.modal)
	}
	if len(rs.roots) < 1 {
		t.Fatal("root switcher should have at least 1 entry (current session)")
	}
	if rs.roots[0].id != "test-session" {
		t.Fatalf("roots[0].id = %q, want test-session", rs.roots[0].id)
	}
	// Modal should not have emitted any op
	assertNoAppOp(t, ops)
}

func TestRootSwitcherWorksWithComposerText(t *testing.T) {
	m, ops := newAppTestModel([]string{"build", "plan"}, nil)
	m.sessionID = "s1"
	...
	// Composer text preserved while modal is open
	if m.composer.Value() != "some text in the composer" {
		t.Fatalf("composer value = %q", m.composer.Value())
	}
	assertNoAppOp(t, ops)
}

func TestRootSwitcherEscCloses(t *testing.T) {
...
func TestRootSwitcherNumberShortcut(t *testing.T) {
	// With a single root, pressing "1" sends activateRootMsg.
	m, _ := newAppTestModel([]string{"build"}, nil)
	m.sessionID = "root-1"
