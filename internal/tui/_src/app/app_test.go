...
func TestPaletteHelpInvocationOpensHelpModal(t *testing.T) {
	...
	for _, want := range []string{"/session", "/rename", "/export", "/copy", "/theme", "/memory", "/queue", "/issues", "/compact", "/fast", "/think", "/layout", "/md-read", "/keys", "/legend", "/settings", "/exit", "/quit", "/palette", "/interrupt", "/agent-next", "/focus-left"} {
