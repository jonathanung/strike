...
func TestHelpModalListsCatalogAndFilters(t *testing.T) {
	...
	for _, want := range []string{"/session [id]", "/export [path] [--open]", "/theme [name|dark|light|auto]", "/memory [list|get|set|rm|export|import] ...", "/issues [list|add|get|close|export|import] ...", "/compact", "/fork", "/undo [chat|files]", "/fast [on|off]", "/think [on|off]", "/layout", "/md-read <path|@path>", "/keys [reset]", "/settings", "/exit", "/quit", "/review $ARGUMENTS", "tab"} {
		found := false
