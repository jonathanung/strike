...
func TestHelpModalListsCatalogAndFilters(t *testing.T) {
	...
	for _, want := range []string{"/session [id]", "/rename [title]", "/export [path] [--open]", "/copy", "/theme [name|dark|light|auto]", "/memory [list|get|set|rm|export|import] ...", "/issues [list|add|get|close|export|import] ...", "/compact", "/fork", "/undo [chat|files]", "/rewind [turn]", "/fast [on|off]", "/think [on|off]", "/layout", "/md-read <path|@path>", "/keys [reset]", "/legend", "/settings", "/exit", "/quit", "/palette", "/interrupt", "/agent-next", "/focus-left", "/review $ARGUMENTS", "tab"} {
		found := false
