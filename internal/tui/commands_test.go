...
func TestValidSkillNameRejectsReservedBuiltinsIncludingMDRead(t *testing.T) {
	for _, name := range []string{"provider", "model", "effort", "autonomy", "auth", "agent", "fast", "vim", "md-read", "theme", "layout", "split", "compact", "fork", "undo", "rewind", "session", "help", "keys", "memory", "issues", "context", "effective-prompt", "cost", "upgrade"} {
