...
func rootActor(tc *Context) (rootID string, err error) {
	if tc == nil {
...
func planResultJSON(v any, title string) (Result, error) {
...
func planSoftError(title, msg string, extra map[string]any) (Result, error) {
...
func planConflictResult(store PlanStore, id string, cause error) (Result, error) {
...
func NewPlanWrite(store PlanStore) Tool {
...
func (t *planWriteTool) Execute(ctx context.Context, args json.RawMessage, tc *Context) (Result, error) {
	if t.store == nil {
	...
	case "create":
		...
		if title == "" {
			return Result{}, errors.New("title is required for create")
		}
	...
	case "update_title":
		...
		if id == "" {
			return Result{}, errors.New("id is required for update_title")
		}
		if !titleSet || strings.TrimSpace(in.Title) == "" {
			return Result{}, errors.New("title is required for update_title")
		}
		if in.ExpectedVersion < 1 {
			return Result{}, errors.New("expected_version is required for update_title")
		}
	...
	case "update_section":
		...
		if id == "" {
			return Result{}, errors.New("id is required for update_section")
		}
		if secID == "" {
			return Result{}, errors.New("section_id is required for update_section")
		}
		if !titleSet && !bodySet {
			return Result{}, errors.New("provide title and/or body for update_section")
		}
		if in.ExpectedVersion < 1 {
			return Result{}, errors.New("expected_version is required for update_section")
		}
	...
	case "add_section":
		id := strings.TrimSpace(in.ID)
		if id == "" {
			return Result{}, errors.New("id is required for add_section")
		}
		if !titleSet || strings.TrimSpace(in.Title) == "" {
			return Result{}, errors.New("title is required for add_section")
		}
		if in.ExpectedVersion < 1 {
			return Result{}, errors.New("expected_version is required for add_section")
		}
		body := ""
		if bodySet {
			body = in.Body
	...
	case "set_status":
		...
		if id == "" {
			return Result{}, errors.New("id is required for set_status")
		}
		if status == "" {
			return Result{}, errors.New("status is required for set_status")
		}
		if in.ExpectedVersion < 1 {
			return Result{}, errors.New("expected_version is required for set_status")
		}
	...
	case "reopen":
		id := strings.TrimSpace(in.ID)
		if id == "" {
			return Result{}, errors.New("id is required for reopen")
		}
		if in.ExpectedVersion < 1 {
			return Result{}, errors.New("expected_version is required for reopen")
		}
		p, err := t.store.Reopen(id, ownerRoot, in.ExpectedVersion)
		return t.finishMutation(id, p, err, "reopened")

	default:
		return Result{}, fmt.Errorf("action must be create, update_title, update_section, add_section, set_status, or reopen")
	}
}

func (t *planWriteTool) finishMutation(id string, p plan.Plan, err error, verb string) (Result, error) {
	if err != nil {
...
func NewPlanRead(store PlanStore) Tool {
...
func (t *planReadTool) Execute(ctx context.Context, args json.RawMessage, tc *Context) (Result, error) {
	if t.store == nil {
		return Result{}, errors.New("plan store is unavailable")
	}
	var in struct {
