...
func TestExitPlanModeHandoff(t *testing.T) {
	var got PlanHandoffRequest
	tc := allowAll(t.TempDir())
	tc.HandoffPlan = func(ctx context.Context, req PlanHandoffRequest) (PlanHandoffResult, error) {
		got = req
...
func TestExitPlanModeHandoffDeclined(t *testing.T) {
	tc := allowAll(t.TempDir())
	tc.HandoffPlan = func(ctx context.Context, req PlanHandoffRequest) (PlanHandoffResult, error) {
		return PlanHandoffResult{}, &UserRejectedError{
	...
	if _, ok := err.(*UserRejectedError); !ok {
...
func TestExitPlanModeExplicitOrchestrator(t *testing.T) {
	...
	tc.HandoffPlan = func(ctx context.Context, req PlanHandoffRequest) (PlanHandoffResult, error) {
		got = req
		return PlanHandoffResult{Agent: req.Agent, ViaPhase: true, ApprovalSource: "agent"}, nil
	}
	_, err := NewExitPlanMode().Execute(context.Background(), mustJSON(t, map[string]any{
		"agent":       "orchestrator",
...
func TestExitPlanModeHeuristicSteps(t *testing.T) {
	...
	tc.HandoffPlan = func(ctx context.Context, req PlanHandoffRequest) (PlanHandoffResult, error) {
		got = req
		return PlanHandoffResult{Agent: req.Agent, ApprovalSource: "agent"}, nil
	}
	_, err := NewExitPlanMode().Execute(context.Background(), mustJSON(t, map[string]any{
		"steps":       5,
...
func TestEnterPlanModePermissionDenied(t *testing.T) {
	tc := &Context{
		Ask: func(ctx context.Context, req AskRequest) error {
			return &CodedError{Code: CodePermissionDenied, Message: "no"}
		},
