...
func TestTUISnapshotPermissionDenied(t *testing.T) {
	tc := &Context{
		...
		TUISnapshot: func(context.Context, TUISnapshotRequest) (TUISnapshotResult, error) {
			t.Fatal("should not run")
			return TUISnapshotResult{}, nil
		},
	}
...
func TestTUISnapshotInvalidJSON(t *testing.T) {
	tc := allowAll(t.TempDir())
	tc.TUISnapshot = func(context.Context, TUISnapshotRequest) (TUISnapshotResult, error) {
		return TUISnapshotResult{Text: "x"}, nil
	}
...
func TestTUISnapshotContract(t *testing.T) {
	c := LookupContract(NewTUISnapshot())
	if c.SideEffect != SideEffectNone || c.Idempotency != IdempotencySafeRetry {
		t.Fatalf("contract = %+v", c)
	}
	if !IsDeferredTool("tui_snapshot") || IsCoreTool("tui_snapshot") {
