...
func TestRejectionInterruptsTurn(t *testing.T) {
	...
	// Inside-workspace destructive path so permission ask still fires; the
	// static path guard blocks some escapes like `rm -rf /` before Ask.
...
complete:
	...
	for _, name := range []string{"model", "autonomy", "perm_mode", "agent", "user", "titled", "started", "begin", "asked", "resolved", "end", "text", "completed", "proc_start", "proc_exit"} {
		if counts[name] != 1 {
...
func TestShutdownDropsBlockedBeginWithoutUnmatchedEnd(t *testing.T) {
	...
	defer cancel()
	go eng.Run(ctx)
	// Drain startup: ModelSelected, AutonomySelected, AgentSelected, PermissionModeSelected.
	for range 4 {
