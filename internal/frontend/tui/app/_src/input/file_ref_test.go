...
func TestFindFileRefSpans(t *testing.T) {
	tests := []struct {
		...
		{
			name: "simple",
			in:   "see internal/frontend/tui/app.go:42 for details",
			want: []fileRef{{Path: "internal/frontend/tui/app.go", Line: 42}},
		},
		{
			name: "with column",
		...
		{
			name: "grep style",
			in:   "internal/frontend/tui/cells.go:99: func render",
			want: []fileRef{{Path: "internal/frontend/tui/cells.go", Line: 99}},
		},
		{
			name: "multiple",
...
func TestFileRefAtColumn(t *testing.T) {
	line := "prefix internal/frontend/tui/app.go:42 suffix"
	col := ansi.StringWidth("prefix ")
	ref, ok := fileRefAtColumn(line, col)
	if !ok || ref.Path != "internal/frontend/tui/app.go" || ref.Line != 42 {
