...
func TestAtFileCompletionActivation(t *testing.T) {
	paths := []string{"internal/frontend/tui/app.go", "pkg/main.go"}
	tests := []struct {
		name     string
		value    string
		row, col int
...
func TestApplyFileCompletionInsertsMention(t *testing.T) {
	ff := &fakeFiles{files: map[string][]byte{
		"internal/frontend/tui/app.go": []byte("package tui"),
		"pkg/main.go":                  []byte("package main"),
	}}
	m, _ := newAppTestModel(nil, nil)
	m.services.Files = ff
	...
	for i, c := range m.completion.Candidates {
		if c.Path == "internal/frontend/tui/app.go" {
	...
	if !strings.Contains(got, "@internal/frontend/tui/app.go") {
		t.Fatalf("composer = %q, want @internal/frontend/tui/app.go", got)
	}
	if m.completion != nil {
