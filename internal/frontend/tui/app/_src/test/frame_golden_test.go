...
func TestFrameGolden(t *testing.T) {
	...
	dir := filepath.Join(moduleRoot(t), "internal", "frontend", "tui", "app", "testdata", "frames")
	cases := []struct {
		file          string
		width, height int
		build         func(*Model)
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("read golden %s: %v (run UPDATE_GOLDEN=1 go test ./internal/frontend/tui/app/ -run TestFrameGolden)", path, err)
			}
			if got != string(want) {
