...
func TestStyleBoundary(t *testing.T) {
	...
	for _, scope := range []struct{ name, dir string }{
		{"root", filepath.Join(root, "internal", "frontend", "tui", "app")},
		{"ui", filepath.Join(root, "internal", "frontend", "tui", "ui")},
	} {
...
func TestStyleBoundaryFixtures(t *testing.T) {
	root := filepath.Join(moduleRoot(t), "internal", "frontend", "tui", "app", "testdata", "style_boundary")
		...
		ast.Inspect(f, func(n ast.Node) bool {
			...
			for _, field := range fn.Type.Params.List {
				for _, name := range field.Names {
					if o := info.Defs[name]; o != nil && namedTypePath(o.Type(), "Theme") == modulePath+"/internal/frontend/tui/theme" {
...
func expressionProvenance(e ast.Expr, info *types.Info, p map[types.Object]styleProvenance) styleProvenance {
	...
	case *ast.CallExpr:
		...
		if s, ok := x.Fun.(*ast.SelectorExpr); ok && s.Sel.Name == "Default" {
			if o := info.Uses[s.Sel]; o != nil && o.Pkg() != nil && o.Pkg().Path() == modulePath+"/internal/frontend/tui/theme" {
				return provenanceResolvedTheme
			}
		}
	case *ast.SelectorExpr:
