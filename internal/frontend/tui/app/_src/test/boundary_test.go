...
//
//   - internal/frontend/tui/** may import no internal/* package other than protocol,
//     frontend/host, and frontend/tui/... (so the frontend never touches auth/config/models/
//     history directly and can be developed against fakes). String redaction
//     uses pkg/redact (not under internal/).
//   - internal/frontend/host (the contract package, not host/local) imports the
...
//   - no backend package (internal/* except internal/frontend/tui/**) imports
//     internal/frontend/tui/**.
//   - harness/** may not import strike-cli/internal/* (#1208).
//   - internal/frontend/tui/** may not import harness/* (stays on protocol + host).
...
// the import rewrites — edit internal/frontend/tui/app/_src/ only, then go generate.
...
func boundaryViolation(pkgDir, imp string) string {
	...
	tuiPrefix := modulePath + "/internal/frontend/tui"
	...
	case isTUIDir(pkgDir):
		if strings.HasPrefix(imp, modulePath+"/harness") {
			return "internal/frontend/tui may not import harness (stays on protocol + host)"
		}
		...
		if suffix == "protocol" || suffix == "frontend/host" || suffix == "frontend/tui" || strings.HasPrefix(suffix, "frontend/tui/") {
			return ""
		}
		return "internal/frontend/tui may only import internal/{protocol,frontend/host,frontend/tui/...}"

	case pkgDir == "internal/frontend/host":
		if strings.Contains(imp, ".") {
			return "internal/frontend/host is a stdlib-only contract package"
		}
		return ""
	...
	case strings.HasPrefix(pkgDir, "internal/") && !isTUIDir(pkgDir):
		if imp == tuiPrefix || strings.HasPrefix(imp, tuiPrefix+"/") {
			return "backend packages must not import internal/frontend/tui"
		}
		return ""
	}
	return ""
...
func isTUIDir(pkgDir string) bool {
	return pkgDir == "internal/frontend/tui" || strings.HasPrefix(pkgDir, "internal/frontend/tui/")
}

func isKitDir(pkgDir string) bool {
	switch pkgDir {
	case "internal/frontend/tui/ui", "internal/frontend/tui/theme", "internal/frontend/tui/common", "internal/frontend/tui/term":
		return true
	}
	return strings.HasPrefix(pkgDir, "internal/frontend/tui/ui/") ||
		strings.HasPrefix(pkgDir, "internal/frontend/tui/theme/") ||
		strings.HasPrefix(pkgDir, "internal/frontend/tui/common/") ||
		strings.HasPrefix(pkgDir, "internal/frontend/tui/term/")
}
...
func kitImportViolation(pkgDir, imp string) string {
	...
	for _, p := range []string{
		modulePath + "/internal/frontend/tui/ui",
		modulePath + "/internal/frontend/tui/theme",
		modulePath + "/internal/frontend/tui/common",
		modulePath + "/internal/frontend/tui/term",
	} {
		if imp == p || strings.HasPrefix(imp, p+"/") {
			return ""
		}
	}
	if strings.HasPrefix(pkgDir, "internal/frontend/tui/term") {
		switch imp {
		case "github.com/creack/pty", "github.com/hinshun/vt10x":
			return ""
		}
	}
	if strings.Contains(imp, "/internal/protocol") || strings.Contains(imp, "/internal/frontend/host") {
...
func TestKitHasNoLogo(t *testing.T) {
	root := moduleRoot(t)
	uiDir := filepath.Join(root, "internal", "frontend", "tui", "ui")
...
func TestKitImportViolation(t *testing.T) {
	cases := []struct {
		...
		{"internal/frontend/tui/ui", "fmt", ""},
		{"internal/frontend/tui/ui", "charm.land/lipgloss/v2", ""},
		{"internal/frontend/tui/ui", "github.com/charmbracelet/x/ansi", ""},
		{"internal/frontend/tui/ui", modulePath + "/internal/frontend/tui/theme", ""},
		{"internal/frontend/tui/ui", modulePath + "/internal/frontend/tui/common", ""},
		{"internal/frontend/tui/term", "github.com/creack/pty", ""},
		{"internal/frontend/tui/term", "github.com/hinshun/vt10x", ""},
		{"internal/frontend/tui/ui", modulePath + "/internal/protocol", "kit packages must not import protocol or host"},
		{"internal/frontend/tui/theme", modulePath + "/internal/frontend/host", "kit packages must not import protocol or host"},
		{"internal/frontend/tui/common", modulePath + "/harness/engine", "kit packages may only import stdlib, Charm, sibling kit, and term's PTY stack"},
		{"internal/frontend/tui/common", modulePath + "/internal/engine", "kit packages may only import sibling kit packages among internal/*"},
		{"internal/frontend/tui/ui", "golang.org/x/sys/unix", "kit packages may only import stdlib, Charm, sibling kit, and term's PTY stack"},
		{"internal/frontend/tui/ui", "github.com/creack/pty", "kit packages may only import stdlib, Charm, sibling kit, and term's PTY stack"},
	}
