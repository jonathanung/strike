...
func TestVisualizerWindowWidthSafe(t *testing.T) {
	...
	msg := visualizerStateMsg{
		...
		FilesTouched: []string{"internal/frontend/tui/app/_src/window/visualizer_window.go", "pkg/protocol/protocol.go", "a/b/c/d/e/f/g/h/i/j/k/long.go"},
...
func TestVisualizerVerificationAndConflictWidthSafe(t *testing.T) {
	msg := visualizerStateMsg{
		...
		PathOverlaps: []visualizerPathOverlap{
			{
				Path:    "internal/frontend/tui/app/_src/window/visualizer_window.go",
				Policy:  "block",
				Blocked: true,
