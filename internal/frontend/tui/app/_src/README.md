...
**Edit Go files here only.** `go generate ./internal/frontend/tui/app` (also `make test` /
`make build` / CI) flattens these into `internal/frontend/tui/app/*.go` because Go allows
only one directory per package.

Flattened `internal/frontend/tui/app/*.go` files are **gitignored** and stamped
