...
func TestSearchFilesRanksAndSkipsHeavyDirs(t *testing.T) {
	...
	write("alpha.go", "a")
	write("internal/frontend/tui/app.go", "app")
	write("internal/frontend/tui/completion.go", "c")
	...
	if len(got) == 0 || got[0] != "internal/frontend/tui/app.go" {
		t.Fatalf("SearchFiles(app) = %v, want internal/frontend/tui/app.go first", got)
	}
	for _, p := range got {
	...
	// Files + parent dirs (internal/, internal/frontend/, internal/frontend/tui/).
	wantFiles := map[string]bool{
		"alpha.go": true, "internal/": true, "internal/frontend/": true, "internal/frontend/tui/": true,
		"internal/frontend/tui/app.go": true, "internal/frontend/tui/completion.go": true,
	}
...
func TestSearchFilesExactPathHitNested(t *testing.T) {
	work := t.TempDir()
	p := filepath.Join(work, "internal", "frontend", "tui", "app.go")
	if err := os.MkdirAll(filepath.Dir(p), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	files := NewFiles(work)
	got, err := files.SearchFiles("internal/frontend/tui/app.go", 5)
	if err != nil {
		t.Fatal(err)
	}
	if len(got) == 0 || got[0] != "internal/frontend/tui/app.go" {
		t.Fatalf("exact nested path = %v, want internal/frontend/tui/app.go first", got)
	}
}
