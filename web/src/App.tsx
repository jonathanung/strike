...
export default function App() {
  ...
  const completions = useMemo(() => {
    ...
    if (token.startsWith("@") && boot?.capabilities.files && Array.isArray((projectData as { entries?: unknown[] })?.entries)) return ((projectData as { entries: Array<{ Name?: string; name?: string }> }).entries || []).map((entry) => entry.Name || entry.name || "").filter((name) => name.toLowerCase().includes(token.slice(1).toLowerCase())).map((name) => ({ label: `@${name}`, detail: "Workspace file", insert: `@${name}` }));
    return [];
  }, [draft, boot, projectData]);
  ...
  const inspectProject = async (tab: InspectorTab) => {
    setInspector(tab); setInspectorOpen(true);
    if (tab === "files") setProjectData(boot?.capabilities.files ? await request("/v1/files?path=").catch((error) => ({ error: error.message })) : undefined);
    if (tab === "project") setProjectData((boot?.capabilities.memory || boot?.capabilities.issues) ? { memory: boot.capabilities.memory ? await request("/v1/memory").catch((e) => ({ error: e.message })) : undefined, issues: boot.capabilities.issues ? await request("/v1/issues").catch((e) => ({ error: e.message })) : undefined } : undefined);
  };
