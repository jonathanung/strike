...
type InspectorTab = "context" | "files" | "memory" | "issues";
...
const slashCommands: Completion[] = [
  ...
  { label: "/rewind", detail: "Rewind the last completed turn", insert: "/rewind" },
...
export default function App() {
  ...
  return <div className="app-shell" style={shellStyle}>
    ...
    <aside className={`inspector ${inspectorOpen ? "open" : "collapsed"}`} aria-label="Inspector"><PanelResize label="Resize inspector panel" value={inspectorWidth} min={240} max={520} onChange={setInspectorWidth} /><div className="inspector-tabs" role="tablist">{(["context", "files", "memory", "issues"] as InspectorTab[]).map((tab) => <button role="tab" aria-selected={inspector === tab} key={tab} onClick={() => void inspectProject(tab)}>{tab}</button>)}</div><div className="inspector-body"><InspectorBody tab={inspector} boot={boot} status={state.status} data={projectData} loading={projectLoading} expandedDiffs={expandedDiffs} toggleDiff={toggleDiff} /></div></aside>
