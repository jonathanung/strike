...
func TestToolFeedbackHelpers(t *testing.T) {
	cases := []struct {
		...
		{"canceled partial empty", ToolFeedbackCanceledPartial(""), "Tool call canceled because the turn was interrupted."},
		{"canceled partial", ToolFeedbackCanceledPartial("hello\n"), "hello\n(incomplete: tool call canceled because the turn was interrupted.)"},
		{"timeout empty", ToolFeedbackTimeout(""), "Tool call timed out."},
		{"timeout detail", ToolFeedbackTimeout("after 2s"), "Tool call timed out after 2s."},
