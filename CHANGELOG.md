...
- **Undo harness edits UX** — `/undo` picker previews harness paths from the last turn (create/update/delete), surfaces checkpoint-skipped counts, and warns when the turn had uncovered mutations (bash). `SessionRewound` carries restored path list + `uncovered` so notices never claim silent full success after shell edits. Multi-file restore stays path-sorted. Wire version `1.11.0`. Full bash snapshot coverage remains #572; checkpoint stack across `--continue` remains #573 ([#801](https://github.com/jonathanung/strike/issues/801)).
- **Protocol forward-compat for harness events** — unknown event type strings decode as `UnknownEvent` (session/TUI/SDK no longer treat them as corrupt); golden additive-field tests; normative [docs/protocol.md](docs/protocol.md) lists harness/verification/permission/hook events, compatibility policy, and plugin/hook contribution paths (links #725/#728). Wire version `1.10.0` ([#811](https://github.com/jonathanung/strike/issues/811)).
- **Agent-to-agent coordination contracts** — `agent_message` gains optional `task_id`, `urgency` (`normal`\|`high`\|`blocker`), `kind` (`message`\|`request`\|`ack`), `require_ack` + `ack_timeout_seconds`, and `escalate_to` (default lead). Un-acked requests emit `agent.contract.timeout` and escalate via mailbox. New `agent_thread` reads task/delegation-bound threads. Wire version `1.6.0` ([#773](https://github.com/jonathanung/strike/issues/773)).
...
- **Sandbox `NoNetwork` field** — host networking is the Go zero value for `sandbox.Policy` (`NetworkEnabled()`); air-gap is opt-in via `NoNetwork` so bare `Policy{Mode, WorkDir}` no longer injects `--unshare-net` ([#750](https://github.com/jonathanung/strike/issues/750)).
...
- Fixed bash under the default OS sandbox failing DNS/network (`gh auth status` misreported as invalid token, `git push`, etc.): `Policy.Network` zero value was air-gap; inverted to `NoNetwork` so defaults and residual call sites keep host networking, with HTTPS integration coverage ([#750](https://github.com/jonathanung/strike/issues/750)).

### Added

- **Network allowlist** — config `network.allow` (global/project) whitelists hosts, `*.suffix` wildcards, IPs, and CIDRs for `webfetch`. Empty means unrestricted public hosts (SSRF private blocks unchanged). Project `allow` replaces global when set. Same policy shape as sandbox `NetworkAllow` / future container net; bash OS network stays all-or-nothing via `NetworkEnabled`. Shown in `/sandbox explain` ([#527](https://github.com/jonathanung/strike/issues/527)).
