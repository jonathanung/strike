...
- **Agent-to-agent coordination contracts** — `agent_message` gains optional `task_id`, `urgency` (`normal`\|`high`\|`blocker`), `kind` (`message`\|`request`\|`ack`), `require_ack` + `ack_timeout_seconds`, and `escalate_to` (default lead). Un-acked requests emit `agent.contract.timeout` and escalate via mailbox. New `agent_thread` reads task/delegation-bound threads. Wire version `1.5.0` ([#773](https://github.com/jonathanung/strike/issues/773)).
